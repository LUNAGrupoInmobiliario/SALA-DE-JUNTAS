...
function autoSetFin() {
  ...
  document.getElementById('f-hora-fin').value = `${hh}:${mm}`;
}
...
function checkConflictosHorario() {
  ...
  if (conflicto) {
    ...
    hint.style.cssText += 'display:block;background:var(--accent-bg);color:var(--accent);';
    hint.style.display = 'block';
  }
}
...
window.submitReservacion = async function () {
  ...
  try {
    ...
    // Reutilizar datos: limpiar solo fecha, horas y propósito
